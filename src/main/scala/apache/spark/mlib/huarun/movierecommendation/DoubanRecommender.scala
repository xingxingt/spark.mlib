package apache.spark.mlib.huarun.movierecommendation
...
object DoubanRecommender {
  ...
  def main(args: Array[String]): Unit = {
    val sc = new SparkContext(new SparkConf().setAppName("DoubanRecommender").setMaster("local[4]"))
    sc.setLogLevel("warn")
    //val base = "/opt/douban/"
    val base = if (args.length > 0) args(0) else "data/"
